      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
           <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md">
              <div className="mb-4">
                 <label className="text-xs font-bold text-slate-500 uppercase mb-1">Ingreso Neto Mensual</label>
                 <input type="number" value={ingresoNeto} onChange={e => setIngresoNeto(Number(e.target.value))} className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-bold dark:text-white" />
              </div>
              <div className="mb-4">
                 <label className="text-xs font-bold text-slate-500 uppercase mb-1">Gastos Fijos Ineludibles</label>
                 <input type="number" value={gastosFijos} onChange={e => setGastosFijos(Number(e.target.value))} className="w-full p-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg font-bold dark:text-white" />
              </div>
              <div className="mb-2">
                 <div className="flex justify-between mb-1">
                    <label className="text-xs font-bold text-slate-500 uppercase">Límite Sano (%)</label>
                    <span className="text-xs font-bold text-blue-600">{limiteSano}%</span>
                 </div>
                 <input type="range" min="10" max="50" step="5" value={limiteSano} onChange={e => setLimiteSano(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700 accent-blue-500" />
                 <p className="text-[10px] text-slate-600 dark:text-slate-400 mt-1">Los expertos recomiendan no superar el 30% de tus ingresos.</p>
              </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
           <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md text-center relative overflow-hidden">
           ...
           <div className="flex-1 bg-slate-50 dark:bg-black p-6 rounded-2xl border border-slate-300 dark:border-slate-800 flex flex-col md:flex-row items-center">
              ...
              <div className="w-full md:w-1/2 space-y-3">
                 {dataChart.map((item) => (
                    <div key={item.name} className="flex items-center justify-between">
                       ...
                       <span className="font-bold text-slate-900 dark:text-white text-sm">{formatMoney(item.value)}</span>
                    </div>
                 ))}
              </div>
           </div>
