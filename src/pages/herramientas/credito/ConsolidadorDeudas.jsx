    ...
    >
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-6 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md">
           <h3 className="text-sm font-bold text-slate-500 uppercase mb-4">Tus Deudas Actuales</h3>
           ...
           <div className="space-y-3 mb-6 max-h-[300px] overflow-y-auto">
              {deudas.map((item) => (
                 <div key={item.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-950 rounded-lg border border-slate-100 dark:border-slate-800">
                    <div>
                       <p className="font-bold text-slate-800 dark:text-white text-sm">{item.nombre}</p>
                    ...
                    <div className="flex items-center gap-3">
                       ...
                       <button onClick={() => eliminarDeuda(item.id)} className="text-slate-600 dark:text-slate-400 hover:text-rose-500"><Trash2 size={16}/></button>
                    </div>
                 </div>
              ))}
           </div>
           ...
           <div className="grid grid-cols-12 gap-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
              <div className="col-span-4">
                 <input type="text" placeholder="Nombre" value={newDeuda.nombre} onChange={e => setNewDeuda({...newDeuda, nombre: e.target.value})} className="w-full p-2 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs" />
              </div>
              <div className="col-span-3">
                 <input type="number" placeholder="Saldo Total" value={newDeuda.saldo} onChange={e => setNewDeuda({...newDeuda, saldo: e.target.value})} className="w-full p-2 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs" />
              </div>
              <div className="col-span-3">
                 <input type="number" placeholder="Cuota Mensual" value={newDeuda.cuota} onChange={e => setNewDeuda({...newDeuda, cuota: e.target.value})} className="w-full p-2 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-xs" />
              </div>
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
           ...
           <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md">
              <h3 className="text-sm font-bold text-slate-500 uppercase mb-4">Nuevo Préstamo Unificado</h3>
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <label className="text-xs font-bold text-slate-500 block mb-1">Monto a Solicitar</label>
                    <div className="text-xl font-black text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-700 pb-1">
                       {formatMoney(analisis.totalDeuda)}
                    </div>
                 </div>
                 <div>
                    <label className="text-xs font-bold text-slate-500 block mb-1">Nueva TNA (%)</label>
                    <input type="number" value={nuevaTNA} onChange={e => setNuevaTNA(Number(e.target.value))} className="w-full font-bold bg-transparent outline-none border-b border-slate-200 dark:border-slate-700 pb-1 dark:text-white" />
                 </div>
                 <div className="col-span-2">
                    <label className="text-xs font-bold text-slate-500 block mb-1">Plazo (Meses): {nuevoPlazo}</label>
                    <input type="range" min="12" max="60" step="6" value={nuevoPlazo} onChange={e => setNuevoPlazo(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700 accent-indigo-600" />
                 </div>
              </div>
           </div>

           {/* Resultado Final */}
           <div className={`flex-1 p-6 rounded-2xl border-2 flex flex-col justify-center items-center text-center ${analisis.conviene ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500' : 'bg-slate-50 dark:bg-slate-900 border-slate-300 dark:border-slate-700'}`}>

              <div className="flex items-center gap-8 mb-6 w-full justify-center">
                 <div>
                    <p className="text-xs font-bold uppercase text-slate-500">Pagas Hoy</p>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">{formatMoney(analisis.totalCuotaActual)}</p>
                 </div>
                 <ArrowRight className="text-slate-600 dark:text-slate-400" />
                 <div>
                    <p className="text-xs font-bold uppercase text-slate-500">Nueva Cuota</p>
