      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
           <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-gray-200 dark:border-slate-800 shadow-sm">
              <h3 className="text-xs font-bold text-gray-500 uppercase mb-4">Paso 1: Entrada</h3>
              
              <div className="mb-4">
                 <label className="text-sm text-gray-700 dark:text-slate-300 font-medium">Capital (USD)</label>
                 <input type="number" value={capitalUSD} onChange={e => setCapitalUSD(Number(e.target.value))} className="w-full mt-1 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 rounded-xl font-bold dark:text-white outline-none focus:border-emerald-500 transition-colors" />
              </div>
              <div className="mb-4">
                 <label className="text-sm text-gray-700 dark:text-slate-300 font-medium">Cotización Venta ($)</label>
                 <input type="number" value={fxEntrada} onChange={e => setFxEntrada(Number(e.target.value))} className="w-full mt-1 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 rounded-xl font-bold dark:text-white outline-none focus:border-emerald-500 transition-colors" />
              </div>

              <h3 className="text-xs font-bold text-gray-500 uppercase mt-8 mb-4">Paso 2: Inversión en Pesos</h3>
              <div className="flex gap-4">
                 <div className="flex-1">
                    <label className="text-xs text-gray-500 font-bold block mb-1">TNA %</label>
                    <input type="number" value={tnaPesos} onChange={e => setTnaPesos(Number(e.target.value))} className="w-full p-2 rounded-lg bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 dark:text-white" />
                 </div>
                 <div className="flex-1">
                    <label className="text-xs text-gray-500 font-bold block mb-1">Días</label>
                    <input type="number" value={dias} onChange={e => setDias(Number(e.target.value))} className="w-full p-2 rounded-lg bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 dark:text-white" />
                 </div>
              </div>

              <h3 className="text-xs font-bold text-gray-500 uppercase mt-8 mb-4">Paso 3: Salida (Estimada)</h3>
              <div>
                 <label className="text-sm text-gray-700 dark:text-slate-300 font-medium">Dólar Futuro ($)</label>
                 <input type="number" value={fxSalidaEsperado} onChange={e => setFxSalidaEsperado(Number(e.target.value))} className="w-full mt-1 p-3 bg-gray-50 dark:bg-slate-950 border border-gray-200 dark:border-slate-700 rounded-xl font-bold dark:text-white outline-none focus:border-emerald-500 transition-colors" />
                 <p className="text-xs text-gray-400 mt-2">¿A cuánto crees que estará el dólar cuando termine la inversión?</p>
              </div>
           </div>
        </div>
        ...
        <div className="lg:col-span-8 space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-emerald-50 dark:bg-emerald-900/10 p-4 rounded-xl border border-emerald-100 dark:border-emerald-900/30 text-center">
                 ...
                 <p className="text-xl font-black text-gray-900 dark:text-white">US$ {capitalUSD}</p>
                 <p className="text-xs text-gray-400">Capital Puro</p>
              </div>
              
              <div className="hidden md:flex items-center justify-center text-gray-300 dark:text-slate-700">
                 <ArrowRight size={32} />
              </div>

              <div className="bg-slate-50 dark:bg-slate-900 p-4 rounded-xl border border-gray-200 dark:border-slate-800 text-center">
              ...
              <div className={`p-4 rounded-xl border-2 text-center ${analisis.yieldPercent > 0 ? 'bg-emerald-50 dark:bg-emerald-900/10 border-emerald-500' : 'bg-rose-50 dark:bg-rose-900/10 border-rose-500'}`}>
                 ...
                 <p className="text-xl font-black text-gray-900 dark:text-white">US$ {analisis.finalUsd.toFixed(2)}</p>
