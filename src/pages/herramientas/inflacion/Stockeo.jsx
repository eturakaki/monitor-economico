      ...
      <div className="grid lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md">
            <div className="mb-4">
              <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Monto Total (Sin Descuento)</label>
            ...
            <div className="space-y-6">
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Descuento Ofrecido</label>
                  <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-bold rounded">{descuento}%</span>
                </div>
                <input type="range" min="0" max="50" step="1" value={descuento} onChange={(e) => setDescuento(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700 accent-blue-600" />
              </div>
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">TNA Billetera</label>
                  <span className="px-2 py-0.5 bg-violet-100 text-violet-700 text-xs font-bold rounded">{tna}%</span>
                </div>
                <input type="range" min="10" max="100" step="1" value={tna} onChange={(e) => setTna(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700 accent-violet-600" />
              </div>
              <div>
                <div className="flex justify-between mb-2">
                  <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Días de Inversión</label>
                  <span className="px-2 py-0.5 bg-slate-100 text-slate-700 text-xs font-bold rounded">{dias} días</span>
                </div>
                <input type="range" min="1" max="60" step="1" value={dias} onChange={(e) => setDias(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer dark:bg-slate-700 accent-slate-500" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-dashed border-slate-300 dark:border-slate-700 flex justify-between items-center">
            <div className="flex items-center gap-2">
               <Banknote size={18} className="text-slate-600 dark:text-slate-400" />
               <p className="text-xs text-slate-500 uppercase font-bold">Monto final a pagar</p>
            </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <div className="flex-1 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-300 dark:border-slate-800 shadow-md min-h-[300px]">
             <h4 className="text-sm font-bold text-slate-900 dark:text-white mb-6">Comparativa de Beneficio ($)</h4>
