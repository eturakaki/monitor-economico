          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            ...
            <div className="w-full lg:w-96 relative group">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <Search className="h-5 w-5 text-slate-600 dark:text-slate-400 group-focus-within:text-emerald-500 transition-colors" />
              </div>
              <input
                type="text"
              ...
              {searchTerm && (
                <button 
                  ...
                  className="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer text-slate-600 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"
                >
                  <X className="h-4 w-4" />
                </button>
              )}
            ...
            Object.entries(toolsGrouped).map(([catKey, tools]) => (
              <section key={catKey} className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex items-center gap-4 mb-6 border-b border-slate-200 dark:border-slate-800 pb-2">
                  <h2 className="text-lg font-bold text-slate-800 dark:text-white uppercase tracking-wider flex items-center gap-2">
  ...
  <button
    ...
    className={`
      ...
      ${isActive 
        ...
        : 'bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-emerald-400 hover:text-emerald-500 dark:hover:text-emerald-400'
      }
    `}
  >
    {label}
...
const ToolCard = ({ tool }) => (
  <Link
    ...
    className={`
      ...
      shadow-md hover:shadow-xl dark:hover:shadow-emerald-900/20
  ...
  >
    ...
    <div>
      <div className="flex justify-between items-start mb-4 relative z-10">
        ...
        {tool.badge && (
          <span className="px-2 py-1 text-xs font-bold uppercase tracking-wider text-slate-500 bg-slate-100 dark:bg-slate-800 dark:text-slate-300 rounded-md border-2 border-slate-200 dark:border-slate-600">
            {tool.badge}
          </span>
        )}
      </div>

      <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-2 relative z-10 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
        {tool.title}
      </h3>
