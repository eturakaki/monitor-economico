...
class VideoErrorBoundary extends React.Component {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-slate-900 text-white animate-in fade-in duration-300 px-4">

          <div className="flex flex-col items-center max-w-sm text-center space-y-4">
            {/* Icono de Estado */}
            <div className="p-4 bg-slate-800 rounded-full shadow-xl ring-1 ring-white/10">
              {isNetworkError ? (
            ...
            <button 
              ...
              className="mt-2 flex items-center gap-2 px-5 py-2.5 bg-white text-slate-900 hover:bg-slate-100 rounded font-medium text-sm transition-colors shadow-lg"
            >
              <RefreshCcw size={16} />
