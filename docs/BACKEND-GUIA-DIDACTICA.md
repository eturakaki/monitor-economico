...
Truco útil: desde Linux podés invocar programas de Windows agregándoles `.exe`. Por ejemplo `explorer.exe .` abre la carpeta actual en el Explorador, y `powershell.exe -Command "..."` ejecuta algo del lado Windows sin cambiar de terminal.
...
Ahora `mon` te lleva al proyecto desde cualquier lado.
...
La solución es siempre la misma: abrir Docker Desktop, esperar a que diga *Engine running*, y **abrir una terminal nueva** — el `PATH` se arma cuando la shell arranca, así que una terminal vieja no ve el comando aunque Docker ya esté andando.
...
Claude Code está instalado **dentro de Ubuntu**, parado en `~/proyectos/monitor-economico`. Es una instalación distinta de la de Windows: son dos sistemas, igual que pasó con Node y con git. La de Windows ve `C:\` y no encuentra el proyecto.
...
Una buena instrucción tiene tres partes: **qué leer**, **qué escribir**, **qué no tocar**.

```
Escribí el archivo backend/README.md, que hoy está vacío.

Antes de escribir, leé estos archivos y basate solo en lo que digan:
backend/docker-compose.yml, backend/pyproject.toml, backend/app/core/config.py.
No inventes comandos ni dependencias que no estén ahí.

Contenido: [apartados concretos]

Restricciones: no modifiques ningún otro archivo, no leas ni escribas
backend/.env, y no hagas commit.
```

### Tres reglas

1. **Leé el diff antes de commitear.** Siempre.
2. **No apruebes permisos en piloto automático.** La lista de `.claude/settings.local.json` debe quedar corta y deliberada, no ser un sedimento de tareas viejas.
3. **Nunca dejes que toque `.env`.**

### La prueba que lo demostró
...
El resto del archivo era bueno: respetó las restricciones, no inventó dependencias que no existieran, y hasta agregó por su cuenta una advertencia útil que no se le había pedido. Pero ese 10% incorrecto cuesta quince minutos de confusión. Multiplicado por cuarenta archivos, es una tarde.

**Y la verificación se hace contra el archivo, no contra el reporte.** Cuando dijo "hechas las dos correcciones", lo que lo confirmó fue:

```bash
grep -n "psql" backend/README.md   # una sola línea, con los valores correctos
```

### La forma del pedido moldea la salida

Los títulos del README salieron numerados ("## 1. Requisitos", "## 5. `.env` nunca se commitea") porque la instrucción enumeraba los apartados. Un README no se lee así. El defecto no era del agente: era del pedido.
...
git diff                     # qué cambió (q para salir del paginador)
gh pr checks 10              # estado del CI de un PR
...
| Qué pasó | Qué enseñó |
|---|---|
| Corrimos comandos de Linux en PowerShell (varias veces) | Son dos sistemas. Mirar el prompt antes de pegar. Conviene poner Ubuntu como perfil predeterminado del terminal. |
...
| "dubious ownership" en `/mnt/c` | Git desconfía de repos de otro usuario porque pueden ejecutar código (hooks). Dar la excepción, mirar, y quitarla. |
...
| `docker` desapareció de Ubuntu | Lo inyecta Docker Desktop. Sin Docker Desktop abierto, no existe. Y hay que abrir una terminal nueva. |
| El `psql` que escribió Claude Code | El resultado de un agente se lee y se verifica contra el archivo, no contra su reporte. |
